...
One way to modell the linearised dynamics around an equilibrium $X^{*}(\mu)$ is via white noise $W_{t}$:
\begin{equation}
        dX_{t}^{(w)} = -\lambda X_{t}^{(w)}dt + \sigma dW_{t}, X_{0}^{(w)} = 0 
...
Here the noise term $\kappa U_{t}dt$ itself is described by an OU-process. There also exist other positive correlated continuous models, but the frequency properties of red noise are the best suited for many applications in Physics as for example the climate system \cite{Hasselmann:1976, Hanggi:1993, Liao:2022}. We will see that the white noise case is a parameter limit of the red noise case. The SDEs for $X_{t}^{(w)}$ and $X_{t}$ can be explicitly solved. We already solved \eqref{eq: white noise linearized SDE} in the section prerequisites. To solve \eqref{eq:red noise linearized SDE} we can for a moment treat $U_{t}$ as a deterministic function of t and use the variations of constants formula for ODE's (Appendix Math Bio): \\
