...
We derive these properties. \cite{gardiner:2009}(page 110) As we have seen already the OU process is a gaussian process. since the Riemann sum of normal random variables converges again to a normal random variable it follows that also $X_{t}$ is a gaussian process. We write \eqref{eq: 3.2a, eq: 3.2b} more compact as a linear system of SDEs: 
\[
...
\begin{subequations}
    \begin{align}
        dX_{t} &= -\lambda X_{t}dt + \kappa U_{t}dt, X_{0} = 0 \\
...
We can derive the solutions for this. 
