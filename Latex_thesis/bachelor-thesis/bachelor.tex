...
After deriving the stationary characteristics of these SDEs we can justify the use of the old EWS more rigorously. Both settings will also be used to test and compare the old methods against the new one.
...
The linearization is assumed to be a good enough approximation to a real fold bifurcation. Since the parameters change in time, the process isn't stationary anymore. Due to the quasi-stationarity argument (see Chapter 3) we can however still apply our methods if the parameters (now including $\kappa$ and $\theta$) change slowly enough.
...
\begin{figure}
    ...
    \caption{AC: ---, Var: - - - \\
    We simulate a red noise driven time series and compute the EWS indicators as described in Chapter 7, only that we know multiplied the deterministic part (the function f) with an $\epsilon = 0.005$, used $\kappa = 0.01$ and observed the time series for a longer time after the TP at $\mu = 2/3$ was reached. It is obvious that all three methods fail in this example.
    }
